...
for (const s of sizes) {
  ...
  await page.screenshot({ path: `output/playwright/${s.name}.png`, fullPage: true });
  report.push({ ...s, ...checks, tiny: checks.tiny.slice(0, 6) });
  await context.close();
}

await browser.close();
writeFileSync('output/playwright/viewport-qa-report.json', JSON.stringify(report, null, 2));
